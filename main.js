...
var srv = http.createServer(function (req, res) {
	...
	switch(params.pathname) {
		case "/announce": 
			...
			getPeers(torrentHash, null, function(peers) {
				console.log(peers);
			...
			addPeer(torrentHash, (params.query.left == 0), peerkey, params.query.peer_id, peer_ip, params.query.port); break;
		...
		//If the tracker requests something we don't support yet:
		default: 
			res.end(bencode.bencode({'failure reason': 'Unhandled request'}).toString()); break;
	}
});
...
function getPeers(torrentHash, count, callback) {
...
//Catch all errors, eventually we'll have to do something with these. 
